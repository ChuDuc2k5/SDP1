...
const assertAuthorized = (currentUser, booking) => {
  ...
  if (currentUser.role === ROLE.CABIN_OWNER) {
    // The current schema has no cabin owner foreign key, so preserve existing access.
    return;
  }
...
const normalizeBookingView = (view) =>
  BOOKING_VIEWS.includes(view) ? view : "upcoming";

const toBookingView = (row) => new Booking(row).toJSON();
...
const getBookingDetailRecord = async (currentUser, bookingId) => {
  if (!isUuid(bookingId)) {
    throw new Error("Booking not found or unauthorized");
  }
...
const getCabinForNewBooking = async (currentUser, cabinId) => {
  ...
  return new Cabin(cabin).toJSON();
};

const createBookingRecord = async (currentUser, payload) => {
  assertCustomer(currentUser);
...
const updateBookingRecord = async (currentUser, bookingId, payload) => {
  const booking = await getBookingDetailRecord(currentUser, bookingId);
...
const cancelBookingRecord = async (currentUser, bookingId) => {
  const booking = await getBookingDetailRecord(currentUser, bookingId);
...
const listBookingsForCurrentUser = async (
  currentUser,
...
const countBookingsForCurrentUser = async (
  currentUser,
...
export const listBookings = async (req, res) => {
  try {
    ...
    const activeView = normalizeBookingView(req.query.view);
    const isCabinOwner = req.currentUser.role === ROLE.CABIN_OWNER;
    const ownerFilters = isCabinOwner ? buildOwnerFilters(req.query) : {};
    const totalBookings = await countBookingsForCurrentUser(req.currentUser, {
      view: activeView,
    ...
    const baseQuery = {
      ...buildBaseQuery(req.query),
      view: activeView,
    };
    const bookings = await listBookingsForCurrentUser(req.currentUser, {
      view: activeView,
      filters: ownerFilters,
    ...
    const upcomingUrl = buildPageUrl("/booking", { ...baseQuery, view: "upcoming" }, 1);
    const historyUrl = buildPageUrl("/booking", { ...baseQuery, view: "history" }, 1);
...
export const showBookingDetail = async (req, res) => {
  try {
    const booking = toBookingView(
      await getBookingDetailRecord(req.currentUser, req.params.id),
    );
    const existingRate = await rateDao.findByBookingId(req.params.id);
    const canRate =
      req.currentUser.role === ROLE.CUSTOMER &&
      booking.userId === getUserId(req.currentUser) &&
...
export const storeBooking = async (req, res) => {
  try {
    await createBookingRecord(req.currentUser, req.body);
